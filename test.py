...
回答要详实专业，可以适当添加例子或相关知识，但不要过度冗长。
        ...
        inputs = self.tokenizer.apply_chat_template(template, add_generation_prompt = True, tokenize = True, return_tensors = 'pt')
...
class BertQuestionAnswering:
    def __init__(self, data_path, use_llm_enhancement=False, llm_model_path="meta-llama/Llama-2-7b-chat-hf", device="cuda"):
    ...
    def get_answers(self, query, top_k_questions=3, enhance_top_answer=True):
        ...
        if self.use_llm_enhancement and enhance_top_answer and all_candidate_answers:
            ...
            enhanced_answer = self.llm_enhancer.enhance_answer(
                query, 
